    ...
    const careerTips = [
      {
        ...
        tips: Array.from(allMissingSkills).slice(0, 5).map(skill => 
    ...
    res.json({
      ...
      recommendations: {
        top_matches: matches.map(match => ({
          ...
          missing_skills: JSON.parse(match.missing_skills)
        })),
...
// GET /api/recommendations/export/:jobId (CSV export for shortlist)
router.get('/export/:jobId', auth, async (req, res) => {
  try {
    const { id: user_id, user_type } = req.user;
    const { jobId } = req.params;

    // Only organisations can export candidates for their jobs
    ...
    const jobQuery = 'SELECT * FROM job_vacancies WHERE vacancy_id = $1 AND org_id = $2';
    ...
    // Generate CSV
    const csvHeaders = ['Name', 'Email', 'Match Score', 'Cosine Score', 'Missing Skills', 'Status'];
    const csvRows = candidates.map(candidate => [
      candidate.name,
      candidate.email,
      candidate.composite_score,
      candidate.cosine_score,
      JSON.parse(candidate.missing_skills).join(', '),
